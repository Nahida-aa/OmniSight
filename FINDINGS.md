...
|---|---|---|
| URL | 42 | 含游戏相关端点 |
| IP:Port | 10 | 硬编码 IP（静态扫描） |

**运行时确认的服务器（通过 PCAP 抓包验证）**:

| 服务器 | IP | 端口 | 协议 | 阶段 |
|---|---|---|---|---|---|
| **大厅/认证服** | `36.155.244.99` | **TCP 65010** | 加密二进制 | 大厅、匹配 |
| **对局数据服** | `**36.150.217.90**` | **UDP 47951** | **UDP（疑似 KCP）** | **对局中** |
| GCloud DNS | `182.254.116.117` | HTTP | HTTPDNS | 全程 |
| 辅助连入 | `36.155.251.15` | — | — | 对局中 |
| 辅助连入 | `36.155.205.230` | — | — | 对局中 |
| 辅助连入 | `36.155.167.223` | — | — | 对局中 |
| 辅助连入 | `36.155.245.16` | — | — | 对局中 |
| 辅助连入 | `36.155.242.39` | — | — | 对局中 |

**所有游戏服务器均在南京，中国移动网络 (AS56046)**

**其他已确认域名**：

| 域名 | IP | 说明 |
|---|---|---|
| `ds-prod-nj-12.df.qq.com` | `58.217.180.240` | 南京游戏服（电信） |
| `ds-prod-nj-12-bak.df.qq.com` | `58.217.182.91` | 南京游戏服备用 |
| `ds-prod-nj-12-bgp.df.qq.com` | `1.13.155.158` | 南京 BGP（移动） |
...
**协议特征**:
- 大厅：**TCP 自定义加密** → 端口 65010
- 对局：**UDP 自定义协议（疑似 KCP）** → 端口 47951
- DNS：GCloud HTTPDNS，域名经 SHA1 哈希后查询
- 全链路加密，无明文数据
- 大厅服到对局服会切换连接

详情: `capture/dfm.pcapng`（17.73 MB 完整抓包）
